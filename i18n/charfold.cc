...
static const CharSetCvt::MapEnt caseFold[] = {
...
// Yuk... Extended plane...
// These are really U+0x10000 + the values below
static const CharSetCvt::MapEnt extendedCaseFold[] = {
0x0400, 0x0428, // DESERET CAPITAL LETTER LONG I
0x0401, 0x0429, // DESERET CAPITAL LETTER LONG E
0x0402, 0x042A, // DESERET CAPITAL LETTER LONG A
0x0403, 0x042B, // DESERET CAPITAL LETTER LONG AH
0x0404, 0x042C, // DESERET CAPITAL LETTER LONG O
0x0405, 0x042D, // DESERET CAPITAL LETTER LONG OO
0x0406, 0x042E, // DESERET CAPITAL LETTER SHORT I
0x0407, 0x042F, // DESERET CAPITAL LETTER SHORT E
0x0408, 0x0430, // DESERET CAPITAL LETTER SHORT A
0x0409, 0x0431, // DESERET CAPITAL LETTER SHORT AH
0x040A, 0x0432, // DESERET CAPITAL LETTER SHORT O
0x040B, 0x0433, // DESERET CAPITAL LETTER SHORT OO
0x040C, 0x0434, // DESERET CAPITAL LETTER AY
0x040D, 0x0435, // DESERET CAPITAL LETTER OW
0x040E, 0x0436, // DESERET CAPITAL LETTER WU
0x040F, 0x0437, // DESERET CAPITAL LETTER YEE
0x0410, 0x0438, // DESERET CAPITAL LETTER H
0x0411, 0x0439, // DESERET CAPITAL LETTER PEE
0x0412, 0x043A, // DESERET CAPITAL LETTER BEE
0x0413, 0x043B, // DESERET CAPITAL LETTER TEE
0x0414, 0x043C, // DESERET CAPITAL LETTER DEE
0x0415, 0x043D, // DESERET CAPITAL LETTER CHEE
0x0416, 0x043E, // DESERET CAPITAL LETTER JEE
0x0417, 0x043F, // DESERET CAPITAL LETTER KAY
0x0418, 0x0440, // DESERET CAPITAL LETTER GAY
0x0419, 0x0441, // DESERET CAPITAL LETTER EF
0x041A, 0x0442, // DESERET CAPITAL LETTER VEE
0x041B, 0x0443, // DESERET CAPITAL LETTER ETH
0x041C, 0x0444, // DESERET CAPITAL LETTER THEE
0x041D, 0x0445, // DESERET CAPITAL LETTER ES
0x041E, 0x0446, // DESERET CAPITAL LETTER ZEE
0x041F, 0x0447, // DESERET CAPITAL LETTER ESH
0x0420, 0x0448, // DESERET CAPITAL LETTER ZHEE
0x0421, 0x0449, // DESERET CAPITAL LETTER ER
0x0422, 0x044A, // DESERET CAPITAL LETTER EL
0x0423, 0x044B, // DESERET CAPITAL LETTER EM
0x0424, 0x044C, // DESERET CAPITAL LETTER EN
0x0425, 0x044D, // DESERET CAPITAL LETTER ENG
0x0426, 0x044E, // DESERET CAPITAL LETTER OI
0x0427, 0x044F  // DESERET CAPITAL LETTER EW
};

static const int caseFoldCnt = sizeof( caseFold ) / sizeof( *caseFold );
static const int extendedCaseFoldCnt =
    sizeof( extendedCaseFold ) / sizeof( *extendedCaseFold );

int
CharSetCvt::Utf8Fold( const StrPtr *s, StrBuf *d )
{
	const unsigned char *t = s->UText();
	const unsigned char *e = s->UEnd();
	int v, l;
	...
	while( t < e )
	{
		...
		case 1:
		    v <<= 6;
		    v += 0xff & *t++;
		    v -= CharSetCvt::offsetsFromUTF8[l];
# ifdef STRICT_UTF8
		    if( v < CharSetCvt::minimumFromUTF8[l] )
		    {
		...
		// v is unicode position
# ifdef STRICT_UTF8
		// check for invalid unicode positions
		if( ( v & 0xfffe ) == 0xfffe || ( v & 0x1ff800 ) == 0xd800 ||
		    ( v >= 0xfdd0 && v <= 0xfdef ) )
		{
		    //lasterr = NOMAPPING;
		    return 1;
...
# endif
		...
		if( v < 0x10000 )
		{
		    unsigned short f = CharSetCvt::MapThru( v,
			caseFold, caseFoldCnt, 0 );
		    if( f )
			v = f;
		    // extend emited multi-byte character
		    if( v >= 0x800 )
		    {
			d->Extend( 0xe0 | ( v >> 12 ) );
			d->Extend( 0x80 | (( v >> 6 ) & 0x3f ) );
		    }
		    else
		    {
			d->Extend( 0xc0 | ( v >> 6 ) );
		...
		{
		    // extended Plane
		    // note we can not fold into here or from here...
		    unsigned short f = CharSetCvt::MapThru( v & 0xffff,
			extendedCaseFold, extendedCaseFoldCnt, 0 );
		    if( f )
			v = f | 0x10000;
		    d->Extend( 0xf0 | ( v >> 18 ) );
		    d->Extend( 0x80 | ( ( v >> 12 ) & 0x3f ) );
		    d->Extend( 0x80 | ( ( v >> 6 ) & 0x3f ) );
		}
		d->Extend( 0x80 | ( v & 0x3f ) );
	    }
	    else
	    {
		// simple ASCII fold
