 ...
 *** THIS IS AN ALTERDED VERSION. It was altered by Sven Erik Knop
 *** sknop@perforce.com, on 14 February 2010, to wrap the functions into a class
 *** and provide error handling through the Perforce Error class.
 *** This also makes regex thread safe, since it does not use global variables
 *** anymore.
 *
...
#define	FAIL(m)	{ error->Set(MsgSupp::RegexError) << m; return(NULL); }
...
/*
 * V8Regex() constructor with not much to do.
 */

V8Regex::V8Regex()
{
	prog = 0;
	regparse = 0;		/* Input-scan pointer. */
	regnpar = 0;		/* () count. */
	regdummy = 0;
...
const char *
V8Regex::matchBegin() const
{
    if( prog )
	return prog[0].startp[0];
    else
	return 0;
}

const char *
V8Regex::matchEnd() const
{
    if( prog )
	return prog[0].endp[0];
    else
	return 0;
}
...
void
V8Regex::compile( const char *exp, Error* e)
{
	error = e;
	...
	if (exp == NULL) {
	    error->Set(MsgSupp::RegexError) << "NULL argument";
	    return;
	}
	...
	if (regsize >= 32767L) {	/* Probably could be 65535L. */
	    error->Set(MsgSupp::RegexError) << "regexp too big";
	    return;
	}

	/* Allocate space. */
	prog = (regexp *)malloc(sizeof(regexp) + (unsigned)regsize);
	if (prog == NULL) {
	    error->Set(MsgSupp::RegexError) << "out of space";
	    return;
	}
...
/*
 - match - match a regexp against a string
 */
	...
	register const char *str, Error * e )
{
	...
	if (prog == NULL || str == NULL) {
		error->Set(MsgSupp::RegexError) << "Null parameter";
		return(0);
	}

	/* Check validity of program. */
	if (UCHARAT(prog->program) != MAGIC) {
		error->Set(MsgSupp::RegexError) << "corrupted program";
		return(0);
	}
...
V8Regex::regmatch( char *prog )
{
	...
	while (scan != NULL) {
		...
		case END:
			...
			break;
		default:
			error->Set(MsgSupp::RegexError) << "memory corruption";
			return(0);
			break;
		}
	 ...
	 */
	error->Set(MsgSupp::RegexError) << "corrupted pointers";
	return(0);
}
...
V8Regex::regrepeat( char *p )
{
	...
	default:		/* Oh dear.  Called inappropriately. */
		error->Set(MsgSupp::RegexError) << "internal foulup";
